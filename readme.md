...
- 在网盘管理中浏览 WebDAV 目录和服务端本地文件夹，并把 WebDAV 文件或目录批量发送给 Aria2。
...
`scrapers.priority` 控制本地刮削的元数据 provider 顺序。配置结构参考 javinizer-go 的多 scraper 设计；当前 JavJaeger 已接入 JavBus provider，其余 provider 配置作为后续接入点保留，并会在刮削诊断日志中标明未接入原因。
