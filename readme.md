...
| 下载 | PikPak API、115 Open API、WebDAV、Aria2 JSON-RPC |
...
PikPak 需要手动登录，或在 `config.json` 中启用 `pikpak` 配置。115 网盘需要在设置页或 `config.json` 中启用 `pan115` 并提供 115 Open API token。Aria2 需要先连接 JSON-RPC，或在 `config.json` 中启用 `aria2` 配置。
...
PUT    /api/system/settings       # 更新 JavBus、WebDAV、Aria2、PikPak 和 115 配置
...
│   ├── pan115/              # 115 Open API offline download dispatch
...
- 115 `access_token` 和 `refresh_token` 只在服务端配置中保存和使用，前端接口只返回是否已配置。
...
例如 WebDAV 至少需要 `url`，Aria2 至少需要 `url`，PikPak 需要 `username` 和 `password`，115 网盘需要 `access_token`。
